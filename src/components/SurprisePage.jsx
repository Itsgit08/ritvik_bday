...
const SurpriseGrid = () => {
  return (
    <div
      className="h-screen w-full grid grid-cols-3 grid-rows-2 gap-4 p-4 bg-cover bg-center"
      style={{ backgroundImage: "url('/images/back14.jpeg')" }}
    >
      {gridItems.map((item, index) => (
        <motion.div
          key={index}
          className="relative flex items-center justify-center overflow-hidden rounded-lg shadow-lg group cursor-pointer h-full"
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-[#5e0c10] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <p className="text-white text-base md:text-lg lg:text-xl font-semibold text-center px-2">{item.message}</p>
          </div>
        </motion.div>
      ))}
